...
function makeReleaseZip($filename, $targetPlattform)
{
	...
	Compress-Archive -LiteralPath $filename, 'Release\lib\', 'README.md', 'help\', 'License.txt', "NppMarkdownPanel\style.css" , "NppMarkdownPanel\style-dark.css", "MarkdownPanelStandalone\bin\Release\MarkdownPanelStandalone.exe" -DestinationPath $zipName -Force
}
